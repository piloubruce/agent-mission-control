...
- **Backend** : Python `server.py` uniquement
...
2. **Ne pas ajouter** de `server.ts`, `server.cjs`, `npm run dev`, ni de proxy Node.
...
Toute modification backend passe par `server.py` uniquement.
...
Le dashboard est accessible sur `http://localhost:51763`.
...
- Le repo doit rester autonome : `server.py` est versionné
