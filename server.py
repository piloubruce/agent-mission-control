...
PROJECT_DIR = os.environ.get("MC_PROJECT_DIR", os.path.expanduser("~/agent-mission-control"))
    ...
    for name in names:
        ...
        if os.path.isfile(p):
            try:
                ...
                for j in jobs:
                    if j.get("id") == job_id:
                        profile = "manager" if name == "default" else name
                        break
            except (OSError, ValueError):
                pass
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_index(self):
        dist_dir = os.path.join(PROJECT_DIR, "dashboard", "dist")
