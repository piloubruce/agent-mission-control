...
import { restartServer, getApiBase } from '../api';
    ...
    <>
      ...
      >
        ...
        <div className={`shrink-0 flex flex-row md:flex-col items-center md:items-stretch gap-1 md:gap-1 md:p-3 md:border-t md:border-stone-800 ${collapsed ? 'md:items-center' : ''}`}>
          {/* Bouton de statut/connexion Backend */}
          <button
            onClick={() => setShowServerModal(true)}
            title={isServerOnline ? 'Hermès Connecté (Cliquez pour configurer)' : 'Hermès Déconnecté (Cliquez pour configurer)'}
            className={`flex items-center gap-2.5 px-3 py-2 text-[11px] font-mono rounded-md transition-colors w-full ${
              isServerOnline
          ...
          >
            ...
            {!collapsed && (
              <span className="truncate flex-1 text-left">
                {isServerOnline ? (customBase ? 'DISTANT · EN LIGNE' : 'LOCAL · EN LIGNE') : 'SERVEUR · CONFIGURER'}
              </span>
            )}
